...
Deno.serve(async (req) => {
  ...
  const refund = await admin.from('refunds').select('id,status,amount_kobo,currency,provider_refund_reference,verified_transaction_id,reconciliation_attempts,requires_manual_review,verified_transactions(provider_reference,currency,amount_kobo)').eq('id', body.refundId).maybeSingle(); if (refund.error || !refund.data) return out({ error: { code: 'not_found' } }, 404); if (refund.data.requires_manual_review) return out({ result: { status: 'pending', requiresManualReview: true } }, 202);
  const transaction = refund.data.verified_transactions;
  const transactionReference = transaction?.provider_reference;
  if (!transactionReference || transaction.currency !== refund.data.currency || Number(transaction.amount_kobo) < Number(refund.data.amount_kobo)) return out({ error: { code: 'reconciliation_invalid_reference' } }, 409);
  const endpoint = refund.data.provider_refund_reference ? `https://api.paystack.co/refund/${encodeURIComponent(refund.data.provider_refund_reference)}` : `https://api.paystack.co/refund?transaction=${encodeURIComponent(transactionReference)}&perPage=50`;
  ...
  const candidates = providerItems.filter((item: any) => Number(item.amount) === Number(refund.data.amount_kobo) && (!item.currency || String(item.currency).toUpperCase() === String(refund.data.currency).toUpperCase()));
  const directRecord = providerData && !Array.isArray(providerData) ? providerData : undefined;
  const record = refund.data.provider_refund_reference
    ? directRecord && String(directRecord.id) === String(refund.data.provider_refund_reference) ? directRecord : undefined
    : candidates.length === 1 ? candidates[0] : undefined;
  if (!record) { const attempts = Number(refund.data.reconciliation_attempts || 0) + 1; const update = await admin.from('refunds').update({ reconciliation_attempts: attempts, requires_manual_review: attempts >= 6, next_reconciliation_at: attempts >= 6 ? null : new Date(Date.now() + Math.min(3600000, 300000 * 2 ** Math.min(attempts, 4))).toISOString(), provider_status: 'not_found_yet', failure_reason: 'provider_outcome_unknown' }).eq('id', refund.data.id); if (update.error) return out({ error: { code: 'reconciliation_failed' } }, 500); return out({ result: { status: 'pending', requiresManualReview: attempts >= 6 } }, 202); }
