...
Deno.serve(async (request) => {
  ...
  if (!event.data?.reference) { const { error } = await admin.from('webhook_receipts').update({ processing_status: 'processed', processed_at: new Date().toISOString(), processing_lease_expires_at: null, next_attempt_at: null }).eq('id', claimed.id); if (error) return out({ error: { code: 'receipt_update_failed' } }, 500); return out({ received: true, processed: true }); }
  ...
  if (applyError) { const { error: updateError } = await admin.rpc('fail_paystack_webhook', { p_receipt_id: claimed.id, p_error: applyError.message }); if (updateError) return out({ error: { code: 'receipt_update_failed' } }, 500); return out({ error: { code: 'payment_application_failed' } }, 500); }
  const { error: processedError } = await admin.from('webhook_receipts').update({ processing_status: 'processed', processed_at: new Date().toISOString(), processing_error: applied?.outcome === 'exception' ? applied.reason : null, processing_lease_expires_at: null, next_attempt_at: null }).eq('id', claimed.id);
