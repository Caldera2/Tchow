...
Deno.serve(async (req) => {
  ...
  const duplicate = received.error?.code === '23505';
  if (received.error && !duplicate) return out({ error: 'receipt_failed' }, 500);
  ...
  const rank = { accepted: 0, delivered: 1, bounced: 2 };
  if (messageId) {
    const current = await db.from('notification_outbox').select('id,provider_status').eq('provider', 'resend').eq('provider_message_id', messageId).maybeSingle();
    if (current.error) return out({ error: 'notification_lookup_failed' }, 500);
    if (current.data && (rank[status as keyof typeof rank] ?? 0) >= (rank[current.data.provider_status as keyof typeof rank] ?? -1)) {
      const updated = await db.from('notification_outbox').update({ provider_status: status }).eq('id', current.data.id);
      if (updated.error) return out({ error: 'notification_update_failed' }, 500);
    }
  }
  return out({ received: true, duplicate });
});
