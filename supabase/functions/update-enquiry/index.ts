...
Deno.serve(async (req) => {
  ...
  const meta = tables[body.kind]; if (!meta || !body.id || (body.status !== undefined && !meta.statuses.includes(body.status))) return out({ error: { code: 'invalid_request' } }, 422);
  ...
  const { data: current, error: readError } = await admin.from(meta.table).select('id,status,internal_notes').eq('id', body.id).maybeSingle(); if (readError) return out({ error: { code: 'read_failed' } }, 500); if (!current) return out({ error: { code: 'not_found' } }, 404);
  const nextStatus = body.status ?? current.status;
  const updates: Record<string, unknown> = { status: nextStatus };
  if (Object.prototype.hasOwnProperty.call(body, 'internalNotes')) updates.internal_notes = String(body.internalNotes || '').slice(0, 10000) || null;
  const { data: record, error } = await admin.from(meta.table).update(updates).eq('id', body.id).select('*').single(); if (error) return out({ error: { code: 'update_failed' } }, 500);
  const audit = await admin.from('audit_events').insert({ actor_id: actor.id, action: current.status === nextStatus ? 'enquiry_note_updated' : 'enquiry_status_changed', entity_type: meta.table, entity_id: body.id, metadata: { from: current.status, to: nextStatus, notesChanged: Object.prototype.hasOwnProperty.call(body, 'internalNotes') } }); if (audit.error) return out({ error: { code: 'audit_failed' } }, 500);
  return out({ record });
});
