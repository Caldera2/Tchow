-- Make the payment RPC report the order state after delivery-capacity triggers run.
-- A successful provider charge is not silently reported as "paid" when the
-- reservation boundary moves the order back to pending for manual review.
...
begin
  ...
  if p_status = 'success' then
    ...
    if o.payment_status = 'paid' then
      return jsonb_build_object('accepted', true, 'outcome', 'paid', 'duplicate', false, 'orderId', o.id, 'paymentStatus', o.payment_status);
    end if;
    return jsonb_build_object('accepted', false, 'outcome', 'exception', 'reason', 'delivery_capacity_unavailable', 'orderId', o.id, 'paymentStatus', o.payment_status);
