...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, nextSession) => { if (mounted) { if (event === 'SIGNED_IN' || event === 'USER_UPDATED') ['tchow-customer', 'tchow-profile', 'tchow-last-order', 'tchow-checkout-attempt', 'tchow-partnership-interest', 'tchow-enquiry', 'tchow-catering-enquiry', 'tchow-partnership-application', 'tchow-contact-enquiry'].forEach((key) => localStorage.removeItem(key)); setSession(nextSession); } });
    return () => { mounted = false; listener.subscription.unsubscribe(); };
  }, []);
  const signOut = async () => { if (!supabase) return { error: new Error('Authentication is not configured.') }; try { const result = await supabase.auth.signOut(); if (!result.error) ['tchow-customer', 'tchow-profile', 'tchow-last-order', 'tchow-checkout-attempt', 'tchow-partnership-interest', 'tchow-enquiry', 'tchow-catering-enquiry', 'tchow-partnership-application', 'tchow-contact-enquiry'].forEach((key) => localStorage.removeItem(key)); return result; } catch (signOutError) { return { error: signOutError }; } };
...
export function AuthGate({ children }) {
  ...
  const navigate = useNavigate(); const currentLocation = useLocation();
  const [redirecting, setRedirecting] = useState(false);
  useEffect(() => { if (!loading && configured && !user && !redirecting) { setRedirecting(true); const destination = `${currentLocation.pathname}${currentLocation.search}`; navigate(`/signin?returnTo=${encodeURIComponent(destination)}`, { replace: true }); } }, [loading, configured, user, redirecting, navigate, currentLocation.pathname, currentLocation.search]);
...
function StaffMfaGate({ children }) {
  ...
  const startEnrollment = async () => { setEnrolling(true); setError(''); try { const result = await supabase.auth.mfa.enroll({ factorType: 'totp', friendlyName: 'Tchow staff authenticator' }); if (result.error) throw result.error; setEnrollment(result.data); } catch (enrollmentError) { setError(enrollmentError?.message || 'Authenticator enrollment could not start.'); } finally { setEnrolling(false); } };
  const verifyEnrollment = async (event) => { event.preventDefault(); if (!enrollment?.id) return; setBusy(true); setError(''); try { const challenge = await supabase.auth.mfa.challenge({ factorId: enrollment.id }); if (challenge.error) throw challenge.error; const result = await supabase.auth.mfa.verify({ factorId: enrollment.id, challengeId: challenge.data.id, code }); if (result.error) throw result.error; setFactor(enrollment); setEnrollment(null); setAssurance({ loading: false, data: { currentLevel: 'aal2', nextLevel: 'aal2' }, error: '' }); } catch (verificationError) { setError(verificationError?.message || 'That enrollment code could not be confirmed. Try again.'); } finally { setBusy(false); } };
  if (!factor && !enrollment) return <main className="page auth-state"><span className="eyebrow">Staff security</span><h1>Authenticator setup required.</h1><p>Privileged staff access requires an enrolled and verified authenticator factor.</p>{error && <p className="form-error" role="alert">{error}</p>}<button className="button" onClick={startEnrollment} disabled={enrolling}>{enrolling ? 'Preparing setup...' : 'Set up authenticator'} <ArrowRight size={17}/></button></main>;
  if (enrollment) return <main className="page auth-page"><div className="auth-card"><span className="eyebrow">Staff security</span><h1>Set up your authenticator.</h1><p>Scan this QR code with an authenticator app, then enter the six-digit code it provides.</p>{enrollment.totp?.qr_code && <img src={enrollment.totp.qr_code} alt="Authenticator setup QR code" className="auth-qr" />}{enrollment.totp?.secret && <p className="muted">Manual setup key: {enrollment.totp.secret}</p>}<form className="wide-form" onSubmit={verifyEnrollment}><label>Authenticator code<input inputMode="numeric" pattern="[0-9]*" autoComplete="one-time-code" value={code} onChange={(event) => setCode(event.target.value)} required /></label>{error && <p className="form-error" role="alert">{error}</p>}<button className="button" disabled={busy}>{busy ? 'Confirming...' : 'Confirm setup'} <ArrowRight size={17}/></button></form></div></main>;
